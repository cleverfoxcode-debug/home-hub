...
│   │       └── OwnerTokenStorage.h/cpp  ← хранение токена владельца (админа)
│   │
│   └── services/
│       ├── bluetooth/
...
public:
    ...
    /// Вызывается при команде get_status — проверка, настроен ли хаб.
    virtual bool onGetStatus() = 0;

    /// Вызывается при команде setup — первичная настройка Wi-Fi.
    ...
    /// Вызывается при команде enable_ap — включение standalone AP-режима.
    /// Хаб создаёт собственную сеть без роутера.
    virtual void enableAccessPoint() = 0;

    /// Вызывается после успешной настройки Wi-Fi.
    /// Provisioning Service отключает рекламу provisioning UUID,
    /// но BLE-стек остаётся активным для дальнейшего использования.
...
**Жизненный цикл Provisioning Service:**
1. При старте без сохранённых Wi-Fi настроек — provisioning активен
2. После успешной настройки (`onSetup` → Wi-Fi подключён) — provisioning отключается
3. BLE-стек продолжает работать для других задач
4. При Factory Reset provisioning активируется снова

**Команды BLE:**
- `get_status` — проверка, настроен ли хаб
- `setup` — передача SSID/пароля, возвращает токен
- `enable_ap` — включение standalone AP-режима (без роутера)

Поля класса | Описание
------------|----------
`onGetStatus()` | Возвращает `true`, если у хаба есть сохранённые Wi-Fi настройки
`onSetup()` | Сохраняет SSID/пароль, генерирует токен, пробует подключиться к Wi-Fi
`enableAccessPoint()` | Включает standalone AP-режим (создаёт сеть `HomeHub-Setup`)
`disableBleProvisioning()` | Отключает provisioning-режим после успешной настройки

**Важно:** 
- PIN-код не используется
- Первичная настройка происходит через Flutter-приложение
- После настройки хаб возвращает токен для регистрации в облаке
- Владелец (админ) определяется на стороне сервера: тот, кто прошёл первичную настройку, становится админом
- Токен сохраняется в NVS (namespace "owner") и используется для авторизации в WebSocket API
- При Factory Reset токен удаляется

Этот интерфейс позволяет `BleProvisioningService` не знать о `WiFiService` напрямую. Все операции с Wi-Fi инкапсулированы в реализации `IBleProvisioningHandler`.
...
#### `OwnerTokenStorage` — хранение токена владельца

**Файлы:** `src/core/utils/OwnerTokenStorage.h`, `src/core/utils/OwnerTokenStorage.cpp`

**Назначение:** Хранилище токена владельца (админа) в NVS. Используется для сохранения токена, полученного при первичной настройке через BLE.
...
public:
    ...
    Result save(const String& token);      // сохранить токен
    Result load(String& outToken);         // загрузить токен
    bool hasOwner() const;                 // есть ли токен
    bool verify(const char* token) const;  // проверить токен
    void clear();                          // удалить токен
};
...
| Ключ | Тип | Описание |
|------|-----|----------|
| `token` | String | Токен владельца (админа) |

**Использование:**
- При первичной настройке через BLE токен сохраняется автоматически
- При Factory Reset токен удаляется
- Используется для авторизации в WebSocket API (будущая функциональность)
...
**Назначение:** Управление Wi-Fi подключением. Реализует конечный автомат с поддержкой сохранения SSID/пароля в NVS, fallback в режим точки доступа, взаимодействие с BLE-провижинингом, mDNS и выводом IP-адреса.
...
void saveStoredConfiguration(const String& ssid, const String& password);  // запись в NVS
